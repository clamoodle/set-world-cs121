...
(function () {
    ...
    const GET_LEADERBOARD_BASE_URL = "/leaderboard";
    ...
    async function addFriend(friend) {
        try {
            ...
            friendLists.forEach(item => {
                if (item.username === friend) {
                    qs("#friends").textContent = item.friends;
                } else {
                    qs("#curr-friends").textContent = item.friends;
                }
            });
    ...
    function populateUserDetails(user) {
        ...
        qs("#friends").textContent = user.friends;
    ...
    /**
     * Fetches the list of top 10 users by high score for displaying them in the leaderboard.
     * @returns {Object} - JSON list of user objects, with rank, username, and high_score.
     */
    async function fetchLeaderboard() {
        try {
            let resp = await fetch(GET_LEADERBOARD_BASE_URL);
            resp = await checkStatus(resp);
            const users = await resp.json();
            return users;
        } catch (err) {
