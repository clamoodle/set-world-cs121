...
(function () {
    "use strict";
    const REGISTER_USER_BASE_URL = "/newUser";
    ...
    async function loginSendCookie() {
        ...
        try {
            let resp = await fetch(LOGIN_BASE_URL, { method: "GET", headers: params });
    ...
    async function registerUser() {
        ...
        try {
            let resp = await fetch(REGISTER_USER_BASE_URL, { method: "POST", body: params });
            resp = await checkStatus(resp);
    ...
    function nextPropertyOption(propIdx, moveByIdx) {
        ...
        for (let i = propIdx + 1; i < properties.length; i++) {
            ...
            switch (currKey) {
                ...
                case "color":
                    properties[i].array =
                        COLOR[TYPE[SPECIES[properties[0].currIdx]][properties[1].currIdx]];
                    break;
            }
